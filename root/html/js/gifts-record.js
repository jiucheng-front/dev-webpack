$(function(){
				...
				if(data.list.length>0){
					...
					$(data.list).each(function (i,obj) {
						...
						if(month!=obj.month||year!=obj.year){
							month = obj.month;
							year = obj.year;
							rowHtml += '<div class="panel-header panel-nav"><div class="panel-title">'+month+'/'+year+'<span class="collect">可提现的余额'+obj.cash_left+'</span></div></div>';
						}
						rowHtml += '<div class="panel-body"><div class="media-list"><ul><li class="media-cell"><div class="media-object pull-left"><span class="media-object-data">'+obj.day+'</span></div><div class="media-content pull-left padding margin-bottom"><div class="gifts-list-detaile"><div class="pull-left margin-left"><h4 class="pull-left">'+obj.ch_name+'提现</h4><span class="media-content-time">'+obj.time+'</span></div><div class="pull-right"><div class="text-right"><span class="media-content-arriva">+'+obj.cash_left+'</span><span class="balance">余额:'+obj.cash_left+'</span></div></div></div></div></li></ul></div></div>'; });
						$('#row').append(rowHtml);
					// $('.scroll-preloader').html('<div class="preloader"></div>');
				}
				else{
					$('.scroll-preloader').html("已经全部显示");
				}
			});
		}, params);
	//result.page//当前页
	...
	function asyncLoadData(callback) {
	    ...
	    $.ajax({
	        url: 'http://t.api.langlive.com/v1/billing/cashier_history',
	        headers: {},
	        type: 'POST',
	...
	asyncLoadData(function (data) {
		...
		if(data.list.length>0){
			...
			$(data.list).each(function (i,obj) {
				...
				rowHtml += '<div class="panel-body"><div class="media-list"><ul><li class="media-cell"><div class="media-object pull-left"><span class="media-object-data">'+obj.day+'</span></div><div class="media-content pull-left padding margin-bottom"><div class="gifts-list-detaile"><div class="pull-left margin-left"><h4 class="pull-left">'+obj.ch_name+'提现</h4><span class="media-content-time">'+obj.time+'</span></div><div class="pull-right"><div class="text-right"><span class="media-content-arriva">+'+obj.cash+'</span><span class="balance">余额:'+obj.cash_left+'</span></div></div></div></div></li></ul></div></div>';
			});
			$('#row').append(rowHtml);
		}
		else{
			$('#row').append('<div class="no-data">暂无数据</div>');
		}
	});
});